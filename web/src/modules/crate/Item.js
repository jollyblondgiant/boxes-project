...
class Item extends PureComponent {
    ...
    constructor(props) {
        ...
        axios.post(routeApi, query({
            ...
            fields: ['rating']}))
            .then(response => {
                console.log("successful response:", crateId)
                if (response.status === 200) {
                     ratings = response.data.data.subscriptionsByCrate
                    let sumRate = ratings.reduce((a, sub) => a + sub.rating, 0);
                    if (ratings.length < 1){average = sumRate} else
                    { average = sumRate / ratings.length };                    
                } else {
                    console.error(response)
                }})
        console.log("constructor:", average, )
        this.state = {
    ...
    render() {
        ...
        { isLoading, isSubbed, crateId, crateRate} = this.state,
        //{id,dddddddddddd name, description} = crate,
...
export default connect(itemState, { getListByUser, create, messageShow, messageHide })(withRouter(Item))
